...
export default function MercuryInteractiveLogo() {
  ...
  useEffect(() => {
      ...
      for (let attempt = 0; attempt < 100; attempt++) {
        ...
        if (data[(y * canvas.width + x) * 4 + 3] > 128) {
          ...
          return {
            ...
            color: 'white', 
            scatteredColor: isInLogoArea ? '#2E5BFF' : '#FFD60A', // Mercury Blue and Gold
    ...
    function createInitialParticles(scale: number) {
      const baseParticleCount = 8000 // Higher density for Mercury logo
    ...
    function animate(scale: number) {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        if (distance < maxDistance && (isTouchingRef.current || !('ontouchstart' in window))) {
          ...
          const moveX = Math.cos(angle) * force * 70
          const moveY = Math.sin(angle) * force * 70
          p.x = p.baseX - moveX
          p.y = p.baseY - moveY
    ...
    <div className="relative w-full h-dvh flex flex-col items-center justify-center bg-black">
      ...
      <div className="absolute bottom-[100px] text-center z-10">
        <p className="font-mono text-gray-400 text-xs sm:text-base md:text-sm">
          Mercury Solutions -{' '}
          <span className="text-mercury-blue-400">Data Analysis</span>{' '}
          &{' '}
          <span className="text-mercury-gold-400">Innovative Technologies</span>
          <br />
          <span className="text-gray-500 text-xs mt-2 inline-block">
            Interactive Logo Experience
          </span>
        </p>
      </div>
    </div>
